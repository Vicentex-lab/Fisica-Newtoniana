...
def crear_triangulo(masas, en_vuelo=False):
    global body, shape, rastro_cm
    if body and shape: 
        space.remove(body, shape) # Si ya existe una figura, se elimina del motor físico
        
    # Limpiar el rastro anterior cada vez que se crea o reinicia el triángulo
    rastro_cm = []
    ...
    if en_vuelo:
        ...
        body.angular_velocity = 8.0 # (en rad / s )
    else:
...
crear_triangulo(masas_vertices, en_vuelo=False)
...
sliders = [
    Slider(x=50, y=140, ancho=200, min_val=1.0, max_val=50.0, val_inicial=1.0, etiqueta="Masa Vértice 1 (Sup)"),
    Slider(x=50, y=210, ancho=200, min_val=1.0, max_val=50.0, val_inicial=1.0, etiqueta="Masa Vértice 2 (Izq)"),
    Slider(x=50, y=280, ancho=200, min_val=1.0, max_val=50.0, val_inicial=1.0, etiqueta="Masa Vértice 3 (Der)")
]
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_ESCAPE:
                ...
                crear_triangulo(masas_vertices, en_vuelo=True)
