...
def integracion_velocidad(body, gravity, damping, dt): #El parámetro amortiguación (damping) se debe pasar a la función cuando hacemos callbacks pese a que no lo usemos.
    """
    ...
    """
    # a = g + (F / m)
    aceleracion = gravity # cabe notar que no consideramos ninguna fuerza más allá de la de gravedad, por tanto, la aceleración es únicamente la de gravedad. 
    
    # v_final = v_inicial  + aceleracion * tiempo 
    body.velocity = body.velocity + aceleracion * dt
...
def integracion_posicion(body, dt):
    ...
    """
    # posicion_final = posicion_inicial + (velocidad * tiempo) NOTA: como usamos el método de Euler-Cromer no consideramos el término de la aceleración, pues es diminuto (t=0,01)
...
class MotorFisica:
    ...
    def crear_figura(self, tipo_figura, masas, velocidades_y, en_vuelo):
        ...
        for i, (v, m, vy) in enumerate(zip(vertices_objetivo, masas, velocidades_y)):
            ...
            # Inyección de las ecuaciones de itinerario explícitas (Asignación de funciones: (Callbacks))
